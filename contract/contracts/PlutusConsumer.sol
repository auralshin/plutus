...
contract PlutusConsumerContract is ERC721, ChainlinkClient, ConfirmedOwner {
    using Strings for uint256;
    using Chainlink for Chainlink.Request;

    uint256 public tokenCounter;
    address private oracle;
    bytes32 private jobIdForMint;
    bytes32 private jobIdForUpdate;
    uint256 private fee;
    ...
    constructor(
        ...
        string memory _jobIdForMint,
        string memory _jobIdForUpdate,
        address _oracle,
        address _link
    ) ERC721(name_, symbol_) ConfirmedOwner(msg.sender) {
        tokenCounter = 0;
        oracle = _oracle;
        jobIdForMint = stringToBytes32(_jobIdForMint);
        jobIdForUpdate = stringToBytes32(_jobIdForUpdate);
        if (_link == address(0)) {
    ...
    function updateJobIDForMint(string memory _jobId) public onlyOwner {
        jobIdForMint = stringToBytes32(_jobId);
    }

    function updateJobIDForUpdate(string memory _jobId) public onlyOwner {
        jobIdForUpdate = stringToBytes32(_jobId);
    }
    ...
    function mint(string memory _userAddress) public {
        address _user = parseAddress(_userAddress);
        ...
        req.add("address", _userAddress);
        req.addUint("tokenId", 0);
        sendChainlinkRequestTo(oracle, req, fee);
    }

    function _setPortfolioValue(uint256 tokenId, PortfolioValue memory uPortfolioValue)
        internal
        virtual
    {
        require(_exists(tokenId), "URI set of nonexistent token");
        _tokenURIs[tokenId] = uPortfolioValue;
    }

    function updateScoreCallback(
        bytes32 _requestId,
        string memory _pValue,
        uint256 _blockNumber,
        uint256 _tokenId
    ...
    function updateReputationScore(string memory _userAddress) public {
        address _user = parseAddress(_userAddress);
        uint256 tokenId = _addressTokenId[_user];
        ...
        Chainlink.Request memory req = buildChainlinkRequest(
            jobIdForUpdate,
            address(this),
            this.updateScoreCallback.selector
        );
        req.add("address", _userAddress);
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    "<defs>",
                    "<style>.cls-1{fill:#00e58c;}.cls-2,.cls-4,.cls-6,.cls-7{fill:#0049e4;}.cls-3,.cls-5,.cls-8{fill:#fff;}.cls-4,.cls-8{font-size:15px;font-family:Montserrat-ExtraBold, Montserrat ExtraBold;font-weight:800;}.cls-5,.cls-7{font-size:6px;}.cls-5,.cls-6,.cls-7{font-family:Montserrat-Bold, Montserrat;font-weight:700;}.cls-6{font-size:5.3px;}</style>",
                    '<clipPath id="clip-path">',
                    '<rect class="cls-1" width="156.32" height="156.32"/>',
                    "</clipPath>",
                    "</defs>",
                    "<title>Reputation_NFT</title>",
                    '<rect class="cls-1" width="156.32" height="156.32"/>',
                    ...
                    '<text class="cls-4" transform="translate(50.95 84.86)">SCORE</text>',
